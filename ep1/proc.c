...
if (rdy_head[USER_Q] == NIL_PROC) { /* Se a fila estiver vazia, */
  rdy_tail[USER_Q] = rp;            /* nao muda nada. */
  rp->p_nextready = rdy_head[USER_Q];
  rdy_head[USER_Q] = rp;
} else { /* insere na lista ordenada pelo tempo */
  struct proc *ant, *atu;
  ant = NIL_PROC;
  atu = rdy_head[USER_Q];
  while ((atu != NIL_PROC) && ((rp->user_time + rp->sys_time) > (atu->user_time + atu->sys_time))) {
    ant = atu; /* percorre ate acabar a lista ou achar um processo com */
    atu = atu->p_nextready; /* tempo maior */
  }
  if (atu != NIL_PROC) { /* nao e o ultimo processo da lista */
    rp->p_nextready = atu;
    if (atu != rdy_head[USER_Q]) ant->p_nextready = rp; /* esta no meio */
    else rdy_head[USER_Q] = rp; /* eh o primeiro da lista */
  } else { /* eh o ultimo processo da lista */
    ant->p_nextready = rp;
    rp->p_nextready = NIL_PROC;
    rdy_tail[USER_Q] = rp;
  }
...
/* Retira o primeiro processo de usuario da lista e o coloca */
/* de volta, segundo a ordem decrescente do tempo. */
  atual = rdy_head[USER_Q]; 
  rdy_head[USER_Q] = atual->p_nextready;
  ready (atual);
