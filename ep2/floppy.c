...
/*???????????????????????????????????????????????????????*/
/*???????????????????????????????????????????????????????*/
PRIVATE u8_t cache[MAX_SECTORS * SECTOR_SIZE];
PRIVATE int cilindro_no_cache = -1,
            cabeca_no_cache   = -1;
/*???????????????????????????????????????????????????????*/
/*???????????????????????????????????????????????????????*/
...
/*???????????????????????????????????????????????????????*/
  		if (r == OK) {
			if (f_opcode == DEV_READ) {
                                if ((fp->fl_cylinder != cilindro_no_cache) ||
                                    (fp->fl_head != cabeca_no_cache))
                                {
                                  cilindro_no_cache = fp->fl_cylinder;
                                  cabeca_no_cache   = fp->fl_head;
                                  setor_ant         = fp->fl_sector;

                                  for (i = 1; i <= f_sectors; i++)
                                  {
                                    fp->fl_sector = i;
							
                                    /* Transfere um setor inteiro para tmp_phys */
                                    tp_aux.tr_count = SECTOR_SIZE;
                                    tp_aux.tr_iop   = NULL;
                                    tp_aux.tr_phys  = vir2phys(NULL);
                                    tp_aux.tr_dma   = tmp_phys;
                                    dma_setup(&tp_aux);
                                    r = f_transfer(fp,&tp_aux);
	
                                    if (r == OK)      /* Realiza a transferencia pro cache */
                                      phys_copy(tmp_phys,
                                                vir2phys(cache + ((unsigned) (i - 1) * SECTOR_SIZE)),
                                                (phys_bytes) SECTOR_SIZE);
                                    else              /* Deu erro */
                                      break;
                                  }
                                  /* Sempre volta ao setor original */
                                  fp->fl_sector = setor_ant;
				}
                                if (r == OK)
                                {
                                  /* Copia do cache pra area de memoria do processo */
                                  phys_copy(vir2phys((cache + ((unsigned) (fp->fl_sector - 1) * SECTOR_SIZE))),
                                            tp->tr_phys,(phys_bytes) tp->tr_count);
                                  fp->fl_sector++;
				}
			}
                        else
                        {
                          ...
                          cilindro_no_cache = cabeca_no_cache = -1;
/*???????????????????????????????????????????????????????*/
/*???????????????????????????????????????????????????????*/
